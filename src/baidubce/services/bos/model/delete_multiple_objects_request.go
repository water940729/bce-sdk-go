...
// Delete multiple objects request definition
package model
...
func (req *DeleteMultipleObjectsRequest) BuildHttpRequest() *http.Request {
    ...
    case *DeleteMultipleObjectsInput:
        ...
        if err != nil {
            req.SetClientError(
                common.NewBceClientError("multi delete json encode fail: " + err.Error()))
            return nil
        }
        httpReq.SetBody(http.NewBodyStreamFromBytes(jsonBytes))
    default:
        req.SetClientError(common.NewBceClientError("invalid delete multi objects body type"))
        return nil
    }
    return httpReq
}
