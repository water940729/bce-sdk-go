...
// put_bucket_acl_request.go - the put bucket acl request definition
...
func (req *PutBucketAclRequest) BuildHttpRequest() *http.Request {
    ...
    case *BucketAclInput:
        ...
        if err != nil {
            req.SetClientError(bce.NewBceClientError("acl json encode fail: " + err.Error()))
            return nil
        }
        httpReq.AddHeader(http.CONTENT_TYPE, "application/json; charset=utf-8")
        httpReq.SetBody(http.NewBodyStreamFromBytes(jsonBytes))
    default:
        req.SetClientError(bce.NewBceClientError("invalid acl parameter type"))
        return nil
    }
    return httpReq
}
