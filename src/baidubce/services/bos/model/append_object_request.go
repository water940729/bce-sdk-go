...
// Append object request definition
package model
...
func (req *AppendObjectRequest) SetOffset(val int64) {
    if val < 0 {
        util.LOGGER.Error().Println("append object offset should be non-negative number:", val)
    }
    req.offset = val
}
...
func (req *AppendObjectRequest) BuildHttpRequest() *http.Request {
    ...
    case *http.BodyStream:
        httpReq.SetBody(val)
    default:
        req.SetClientError(common.NewBceClientError("invalid append object input type"))
        return nil
    }
